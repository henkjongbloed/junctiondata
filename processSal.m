function S = processSal(V, U, ctd, DS)
...
S.S          =  ctd.S; %salinity
adcpT = datenum(V.raw.timeV ) ; % in days
hour = datenum('12-Aug-2014 01:00:00') - datenum('12-Aug-2014 00:00:00');

[adcpH, aindh] = getHour(adcpT, hour/2); % ADCP hours
% WGS2UTM ook bij ADCPData gebruikt?? ->
if strcmp(DS,'NMOMNW15')    % Does not work correctly yet
    [S.X, S.Y]      = wgs2utm(ctd.Y, ctd.X); %Convert LatLon to UTM - pay close attention to coordinate systems. WGS84 - ETRS89 differs at most 65 cm, not relevant
    [ctdH, cindh] = getHour(ctd.T2, hour/2);% + datenum('9-Aug-2014 00:00:00'); % Correct
    S.t = rescale(ctdH, min(adcpH), max(adcpH)); % Everything works perfectly
    xnew = center_cloud([S.X, S.Y], U.xs.origin'); % Because of offset in CTD coordinates relative to ADCP - quick fix
...
elseif strcmp(DS,'OMHA14')
    ...
    ctddt = 6000;
    [ctdH, cindh] = getHour(ctd.T, ctddt); % dt = 6000 -> 1 sample per 6s?
    S.t = rescale(ctdH, min(adcpH), max(adcpH));
    if strcmp(U.BN, ' OMN')
        S.t = manualOMN(ctd.T, adcpH);
    end
...
for i = 2
    epsi = randn(size(S.tsnz_p));
    disp(nl(i))
    tic
    SI1(:,i) = SI.interpolate(S.tsnz_p' + nl(i)*epsi');
    t1 = toc;
    tic
    SI2(:,i) = griddatan(S.tsnz_p, S.S, S.tsnz_p + nl(i)*epsi); %150 times slower than loess in standard config
    t2 = toc;
    nor(i,1) = i;
