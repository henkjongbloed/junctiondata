...
[adcpH, ~] = getHour(adcpT, hour/2); % ADCP hours
...
if strcmp(DS,'NMOMNW15')    % Does not work correctly yet
    ...
    % Time
    [ctdH, ~] = getHour(ctd.T2, hour/2);% + datenum('9-Aug-2014 00:00:00'); % Correct
...
elseif strcmp(DS,'OMHA14')
    ...
    ctddt = 6000;
    [ctdH, ~] = getHour(ctd.T, ctddt); % dt = 6000 -> 1 sample per 6s?
...
S.sig = rescale(z2sig(S.Z, S.zb, S.eta), 0, 1); % IS THIS ALLOWED IN SIGMA COORDINATES?? - TAKE CARE
