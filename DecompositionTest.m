...
X.t = linspace(t0, T, Nt);
X.y = linspace(0, B, Ny);
X.sig = linspace(0, 1, Ns);

[X.T, X.Y, X.Sig] = ndgrid(X.t, X.y, X.sig);
phi = pi/3;

F = sin(2*pi*X.T).*(X.Sig).^2;%0*rand(size(Tt))-1/2; %all variables are of order 1
G = sin(2*pi*X.T - phi).*(1-X.Sig).^2;% 0*rand(size(Tt))-1/2;% - .1*rand(size(Tt));
H = 10*ones(size(X.T)) + sin(2*pi*X.T/T + phi);


D = Decomposition(X=X, H=H);

DF = D.decompose_function(F);

[SF,C] = D.decompose_product(F,G);
