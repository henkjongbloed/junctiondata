function [msh] = compQ_v2(msh, trancsts, nfrac, vfield)
...
% estimate discharge
for ct=trancsts(:)'
    [msh{ct}.cs.A, msh{ct}.p ] = integrate_area(msh{ct}.p, msh{ct}.N, msh{ct}.Z, trancsts,nfrac(ct));
    [msh{ct}.cs.Q, vel] = integrate_discharge(msh{ct}.cs.A, msh{ct}.p, msh{ct}.N, msh{ct}.Z, msh{ct}.(sfield).(vfield));
end % for ct
...
p.A = 0.5*(sum(bsxfun(@times,p.Z([6 1 5 2],:,1),[1 -1 1 -1]'),1).*...
    sum(bsxfun(@times,p.N([2 1],:),[1 -1]'),1)+...
    sum(bsxfun(@times,p.Z([5 2 4 3],:,1),[1 -1 1 -1]'),1).*...
...
% compute discharge
Q.mid   = sum(vel(fgood(:,:,1)).*p.A(:)');
%Q.mid   = sum(vel(fgood(:,:,1)));
%Q.mid   = vel(fgood(:,:,1));
