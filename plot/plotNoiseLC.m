function plotNoiseLC(dat, ld_idx, est_opts)

rp = dat.opts.reg_pars;
SA = reshape(dat.avg_err, length(rp{3}), length(rp{1}),[]);
if strcmp(dat.opts.reg_vary, 'coupled')
    [X,Y] = meshgrid(est_opts.noise_levels, symlog(rp{1}, dat.opts.res_near_zero));
end
...
%     s.XTickLabel = round(symexp(X(1,idxt), dat.opts.res_near_zero), 1, 'significant');
    ...
    title(sprintf('log$_{10}$ error: $\\lambda_d$ = %.2f', rp{3}(fi)), 'interpreter', 'latex')
%     disp(rp{1}(fi))
    %     colormap("jet") 
